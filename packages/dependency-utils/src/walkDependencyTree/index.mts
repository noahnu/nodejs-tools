...
export async function* walkDependencyTree(
  ...
  const visitors = {
    [TSESTree.AST_NODE_TYPES.ImportDeclaration]: (node: TSESTree.ImportDeclaration) => {
      imports.add({
        kind: ImportDescriptorKind.Import,
        source: node.source.value,
      })
    },
    [TSESTree.AST_NODE_TYPES.CallExpression]: (node: TSESTree.CallExpression) => {
      if (
    ...
    [TSESTree.AST_NODE_TYPES.ImportExpression]: (node: TSESTree.ImportExpression) => {
  ...
  for (const body of ast.body) {
    simpleTraverse(body, { visitors } as Parameters<typeof simpleTraverse>[1])
  }
    ...
    try {
      ...
      try {
        ...
        if (!path.extname(request)) {
          for (const extension of ['.ts', '.mts', '.tsx', '.js', '.mjs', '.cjs']) {
            try {
